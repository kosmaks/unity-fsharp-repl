...
let unitylog = UnityEngine.Debug.Log
let exit (_ : int) = ()
let mutable isRunning = true

// Super kostyl
let mutable mtFunction : (unit -> obj) option = None
let mutable mtResult : obj option = None
let mtExec<'a> (fn : unit -> 'a) =
  mtResult <- None
  mtFunction <- Some (fn >> (fun x -> x :> obj))
  let rec iter () =
    match mtResult with
    | Some result -> 
        mtFunction <- None
    ...
    | None -> 
        Thread.Sleep(1)
        if isRunning then iter() else None
  iter()
          ...
          (new Thread(fun () -> 
              ...
              | Some console when fsiOptions.EnableConsoleKeyProcessing && not fsiOptions.IsInteractiveServer ->
                  if isNil fsiOptions.SourceFiles then 
                      firstLine <- Some(inReader.ReadLine()); 
...
type FsiStdinLexerProvider(tcConfigB, fsiStdinSyphon, 
    ...
    let rec waitForValue op = 
      let value = op()
      if isRunning && String.IsNullOrEmpty(value) then 
        Thread.Sleep(1)
        waitForValue op
      else
        value
        ...
        let lexbuf = 
            ...
            | Some console when fsiOptions.EnableConsoleKeyProcessing && not fsiOptions.IsInteractiveServer -> 
                LexbufFromLineReader fsiStdinSyphon (fun () -> 
                    ...
                    | None -> 
                          let line = waitForValue fsiConsoleInput.In.ReadLine
                          line)
                          (* console.ReadLine()) *)
            | _ -> 
              
               LexbufFromLineReader fsiStdinSyphon (fun () -> 
                   let line = waitForValue fsiConsoleInput.In.ReadLine |> removeZeroCharsFromString
                   line)
#endif                
    ...
    let MainThreadProcessParsedInteraction (exitViaKillThread, action, istate) =         
        ...
        |  e ->
           ...
           istate,CompletedWithReportedError
            ...
            let res = mtExec <| fun () -> ExecInteractions (exitViaKillThread, tcConfig, istate, action)
...
type internal FsiEvaluationSession (argv:string[], inReader:TextReader, outWriter:TextWriter, errorWriter: TextWriter) = 
...
#if SILVERLIGHT
    let defaultFSharpBinariesDir = "/usr/local/lib/mono"
    let currentDirectory = "."
#else    
    let currentDirectory = match mtExec <| fun () -> UnityEngine.Application.dataPath with
                           | Some path -> sprintf "%s/../" path // root instead of Assets
                           | None -> "."
    let defaultFSharpBinariesDir = sprintf "%s/References" currentDirectory
#endif
    ...
    let tcGlobals,tcImports = TcImports.BuildTcImports(tcConfigP) 
